...
export async function getTaxDeclarations(orgId: string, financialYear?: string) {
  let query = supabase
    ...
    .select(
      '*, employee:employees(id, first_name, last_name, email, employee_code, department_id, employment_type, department:departments(id, name)), verifier:employees!employee_tax_declarations_verified_by_fkey(id, first_name, last_name)'
    )
